...
template<int N>
void pickPhysicalDevice(const VkPhysicalDevice *const in_list, const uint32_t in_length, const typename std::array<const char* const, N>& reqExtensions, VkPhysicalDevice &out_physicalDevice, int32_t &out_qFamilyIndx) noexcept;
template<int N>
[[gnu::always_inline]] [[gnu::const]] bool physicalDeviceIsSuitable(const VkPhysicalDevice in_physicalDevice, const typename std::array<const char* const, N> in_reqExtensions, int32_t &out_qFamilyIndx) noexcept;
